...
export const login = (email, password) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      ...
      "http://localhost:5000/dev/api/v1/user/login",
      { email, password },
      config
    );
    ...
    const otpData = await axios.post(
      // "http://43.204.65.150/dev/api/v1/user/sendOTP",
...
export const otpVerify = (otp) => async (dispatch) => {
  ...
  let email = "nmcreations07@gmail.com";
  // console.log(email);
  // let newPassword = "nmb1234";
...
export const resendforgotPassword = (email) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      ...
      "http://localhost:5000/api/auth/resendpasswordCode",
      { email },
      config
    );
  ...
  (firstName, lastName, email, password) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(
        ...
        "http://localhost:5000/dev/api/v1/user/register",
...
export const getUserDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(
      // `http://43.204.65.150/dev/api/v1/user/single/${id}`,
      `http://localhost:5000/dev/api/v1/user/single/${id}`,
      config
    );
